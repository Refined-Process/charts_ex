defmodule Charts.ColumnChartTest do
  ...
  describe "columns/1" do
    test "returns the number of columns that make up the dataset" do
      assert length(BarChart.columns(@chart)) == length(@data)
    end

    test "returns column labels" do
      columns = Enum.map(BarChart.columns(@chart), & &1.label)
    ...
    test "returns evenly distributed column widths" do
      column_widths = Enum.map(BarChart.columns(@chart), & &1.width)
    ...
    test "returns bar widths as half of column widths" do
      bar_widths = Enum.map(BarChart.columns(@chart), & &1.bar_width)
    ...
    test "returns correct column offsets" do
      column_offsets = Enum.map(BarChart.columns(@chart), & &1.offset)
    ...
    test "returns bar_offsets with margins taken into account" do
      bar_offsets = Enum.map(BarChart.columns(@chart), & &1.bar_offset)
    ...
    test "calculates column height as a percent of y-axis max value" do
      column_heights = Enum.map(BarChart.columns(@chart), & &1.column_height)
